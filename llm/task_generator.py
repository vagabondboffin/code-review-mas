...
class TaskGenerator:
    ...
    def generate_task(self, temperature=0.7) -> str:
        """Generate a backend feature request using the specified model"""
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user",
                     "content": f"Generate a specific backend feature request for a {random.choice(self.task_types)}. Use technical language and include 1-2 key requirements. Respond with only the feature request text."}
                ],
